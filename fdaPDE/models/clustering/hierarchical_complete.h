...
#ifndef __HIERARCHICAL_COMPLETE_H__
#define __HIERARCHICAL_COMPLETE_H__
...
class HierarchicalComplete : public ClusteringBase<HierarchicalComplete, SpaceOnly> {
private:
    typedef ClusteringBase<HierarchicalComplete, SpaceOnly> Base;
...
public:
    ...
    using This = HierarchicalComplete;
    using Base::df_;
    unsigned k;
    HierarchicalComplete() = default;
    HierarchicalComplete(const Base::PDE& pde, Sampling s, unsigned k)
        : Base(pde, s), k(k) { }
    ...
    double cluster_distance(const std::vector<int>& cluster1, const std::vector<int>& cluster2) {
        ...
        for (int i : cluster1) {
            for (int j : cluster2) {
                sum_dist += norm2(y().row(i) , y().row(j));
    ...
    virtual ~HierarchicalComplete() = default;
};

}  // namespace models
}  // namespace fdapde

#endif  // __HIERARCHICAL_COMPLETE_H__
